...
data {
    ...
    vector[2] data_exp_c;                   // Planck H_0 and q_0 estimates
    matrix[2, 2] cov_l_exp_c;               // Cholesky decomp of cov mat of above estimates
...
model {
    ...
    // expansion priors drawn from tight-ish priors, then
    // cosmological constraints applied
    h_0 ~ normal(70.0, 20.0);
    delta_h_0 ~ normal(0.0, 6.0);
    q_0 ~ normal(-0.5, 1.0);
    delta_q_0 ~ normal(0.0, 0.5);
    {
        vector[2] true_par_exp_c;
        true_par_exp_c[1] = h_0 + delta_h_0;
        true_par_exp_c[2] = q_0 + delta_q_0;
        data_exp_c ~ multi_normal_cholesky(true_par_exp_c,
                                           cov_l_exp_c);
    }
